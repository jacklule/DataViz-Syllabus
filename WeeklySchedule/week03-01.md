...
- [Google Fusion map tutorial with image cards](https://support.google.com/fusiontables/answer/2527132?hl=en) Some annotation: 
  - 1) Google Drive now says "New", not "Create"; 
  - 2) Fusion Tables is under More; 
  - 3) will want to customize to better design the info window; 
  - 4) to change the style of the date to more recognizable format, click on Date> Change> and change to Text: ie., May 24, 1941; 
  - 5) to share data and map, use "Share" but to share map, first go to Tools > Publish > and get an embed code or url.
